...
class CleargaspDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        aligned_gt, s, t = align_scale(gt_depth, init_depth, mask) # (H, W)
        ...
        residual = aligned_gt - init_depth  # (H, W)
        ...
        if normal_img is None:
            normal_img = np.zeros((init_depth.shape[0], init_depth.shape[1], 3), dtype=np.float32)
        ...
        init_depth = cv2.resize(init_depth, (target_w, target_h), interpolation=cv2.INTER_NEAREST)
        ...
        init_depth = np.expand_dims(init_depth, axis=0)       # (1, H, W)
        ...
        sample = {
            # 입력(input): RGB 이미지(3차원) + init_Depth 이미지(1차원) = 4차원
            "input": torch.from_numpy(np.concatenate([rgb, init_depth], axis=0)).float(),  # (4, H, W)
