#  Model 정의 함수
# : 투명 물체 이미지로만 학습 시킨 MDE, 기존 U-Net 네트워크 (ablation 실험용)
...
class UNetBaseline(nn.Module):
    def __init__(self, n_channels=3, n_classes=1):
        """
        Args:
            n_channels: 입력 피처 맵의 채널 수 (RGB = 3)
            n_classes: 출력 피처 맵의 채널 수 (Depth = 1)
        """
        # Encoder 인스턴트 변수 정의
        super(UNetBaseline, self).__init__()
        self.inc = DoubleConv(n_channels, 64) # (3, 288, 512) -> (64, 288, 512)
    ...
    def forward(self, x):
        ...
        Args:
            x: 메인 입력 (RGB = 3채널)
        Returns:
            logits: depth map, (B, 1, H, W)
        """
        # Encoder
        x1 = self.inc(x)        # (Batch, 3, 288, 512) -> (Batch, 64, 288, 512)
